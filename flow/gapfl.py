...
def gap(cst, req, cap):
    ...
    m += z, '目的関数'  # zの実体は以下の制約条件内で定義する
    # set(df.i)はi列の一覧、set(df.j)はj列の一覧。|でつないだら、その和集合
    for n in set(df.i)|set(df.j):
        if n == s:
            m += pulp.lpSum(df.Var[df.i==n]) - pulp.lpSum(df.Var[df.j==n]) == z, f'始点{n}'
        elif n == t:
            m += pulp.lpSum(df.Var[df.i==n]) - pulp.lpSum(df.Var[df.j==n]) == -z, f'終点{n}'
        else:
            m += pulp.lpSum(df.Var[df.i==n]) - pulp.lpSum(df.Var[df.j==n]) == 0, f'点{n}'
